...
class StandXPerpHTTP:
    ...
    def get_region(self) -> RegionResponse:
        ...
        # Add a short timeout to avoid long blocking due to network issues
    ...
    def _get_sign_timestamp(self) -> int:
        """
        Get timestamp (seconds) to use for signing requests.

        Prefer server time; fall back to local time if server time cannot be obtained.
        """
        try:
        ...
        # Fallback: use local time (seconds)
    ...
    def place_order(
        ...
        # Use cached server time or local time for signing to avoid frequent geo API calls causing blocking
